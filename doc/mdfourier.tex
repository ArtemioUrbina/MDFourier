...
The present documents serves as an introduction, a manual and a description of the methods used and how they work.
\end{abstract}
...
The next step is setting up your \textit{audio capture card} and computer, in order to record a \textit{44 or 48khz 16 bit stereo} \define{wav} file.
...
The compromise made is to use \textit{sub second signals} for each element to be compared. Since the time and sample rate determine how the \define{dfft} \textit{frequency bins} are spaced after analysis - and how much information we end up analyzing - the end result is a lower plot resolution.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\linewidth]{plots/Plot3-Spectrogram-500hz.png}
...
The following table lists all the hardware used to make the recordings for the plots that will be shown. All had stock parts at the time of the recording, and used original power supplies. They were all connected to a 4" \textit{CRT} via \textit{RGB}, although the \textit{CRT} was turned off while recording. 

Although the recordings were made using several audio cards as described in section \ref{audiocards}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.0\linewidth]{plots/framerate-scope.png}
